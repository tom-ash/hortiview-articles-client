...
export const getConfig = (token: string, organizationId?: string, method = 'GET'): RequestInit => {
...
export const prependBaseUrl = (
  ...
  const organizationPart = withOrganizationContext && organizationId ? `/${organizationId}` : '';
  ...
  switch (type) {
    ...
    default:
      throw new Error('Invalid api type - needs to be implemented in HortiViewService');
  }
};
...
export const formatQuery = <T>(query?: Query<T>, withApiVersion = true) => {
  ...
  if (overwrite) {
    ...
    const formatted = buildQuery({ expand, filter, orderBy, select, top, skip, count });
    return isEmpty(formatted) ? `${questionMark}${version}` : `${formatted}${ampersand}${version}`;
  }
};
 ...
 */
export const invalidateMultipleQueries = (queryClient: QueryClient, keys: QueryKey[]) => {
