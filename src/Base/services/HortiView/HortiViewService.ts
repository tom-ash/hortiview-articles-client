...
import { formatQuery, getConfig, getMockResponse, getResponse, prependBaseUrl } from './Helper';
...
export class HortiViewService implements IHortiViewService {
  ...
  public testConnection = (): Promise<boolean> => {
    return new Promise(resolve => {
      this.baseFetch('HealthzChecks/ping', ApiType.CommonNoVersion, {}).then(resp => {
  ...
  public baseFetch = <T>(
    ...
    if (HortiViewService.mock || permamock) return getMockResponse(url, query ?? {});
  ...
  public baseMutation = <T, R>(
    ...
    const completeUrl = prependBaseUrl(url, api, this.getApiUrl(api), true, orgId);
    //return only getResponse
