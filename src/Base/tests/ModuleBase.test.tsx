...
describe('ModuleBase', () => {
  ...
  test('render when standalone is true', async () => {
    ...
    expect(
      await screen.findByText('Currently running locally with standalone mode enabled')
    ).toBeInTheDocument();
  });
...
describe('Internal functions', () => {
  ...
  test('checkPropsExist should throw error when required prop is missing', () => {
    expect(() => testExport.checkIfPropExists('config', customProps)).toThrowError();
  });

  test('checkPropsExist should not throw error when required prop is present', () => {
    expect(() =>
      testExport.checkIfPropExists('basePath', { ...customProps, basePath: '/path' })
    ).not.toThrowError();
  });
});
